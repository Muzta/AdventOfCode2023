...
const part1 = () => {
  ...
  const compareHands = (first, second) => {
    const firstHand = first.split(" ")[0].trim();
    const secondHand = second.split(" ")[0].trim();
    const firstStrength = getHandStrength(firstHand);
    const secondStrength = getHandStrength(secondHand);
    if (firstStrength > secondStrength) return 1;
    if (firstStrength < secondStrength) return -1;

    // Same value hands, strongest first card check
    const cardStrengths = [
      "2",
      "3",
      "4",
    ...
    // This is a simple "for" so it can exit the compareHands functino as a whole, not possible with "forEach"
  ...
  const sortedHands = lines.sort(compareHands); // Sort the hands, from weaker to stronger
